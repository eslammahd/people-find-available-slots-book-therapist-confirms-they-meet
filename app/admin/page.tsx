    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          ...
          <div className="text-sm text-slate-500 mt-1">Confirmed</div>
        </div>
        <div className="bg-white rounded-2xl border border-slate-200 p-5">
          <div className="text-3xl font-bold text-slate-700">{bookings.length}</div>
      ...
      <div className="flex gap-2 mb-6">
        {(['pending', 'confirmed', 'all'] as const).map(tab => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            ...
            {tab} {tab === 'pending' && pendingCount > 0 && <span className="ml-1 bg-amber-500 text-white text-xs px-1.5 py-0.5 rounded-full">{pendingCount}</span>}
          </button>
        ))}
      </div>
        ...
        <div className="space-y-4">
          {filtered.map(b => (
            <div key={b.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              {b.notes && <p className="text-sm text-slate-500 bg-slate-50 rounded-lg p-3 mb-3">💬 {b.notes}</p>}
              {b.status === 'pending' && (
