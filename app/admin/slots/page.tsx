...
export default function AdminSlotsPage() {
  ...
  const addSlot = async (e: React.FormEvent) => {
    ...
    const data = await res.json();
    if (!res.ok) { setError(data.error || 'Failed to add slot'); } else { await fetchSlots(); setDate(''); }
    setAdding(false);
  };
        ...
        <form onSubmit={addSlot} className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
          <div className="sm:col-span-2">
            ...
            <input type="date" value={date} onChange={e => setDate(e.target.value)} required min={new Date().toISOString().split('T')[0]}
              className="w-full border border-slate-300 rounded-xl px-3 py-2.5 focus:ring-2 focus:ring-sky-500 outline-none text-sm" />
          </div>
          <div>
          ...
          <button type="submit" disabled={adding} className="sm:col-span-4 bg-sky-600 text-white py-2.5 rounded-xl font-semibold hover:bg-sky-700 transition-colors disabled:opacity-50 text-sm">
            {adding ? 'Adding...' : '+ Add Slot'}
          </button>
        </form>
      </div>
      {loading ? (
        <div className="text-center py-12"><div className="inline-block w-8 h-8 border-4 border-sky-600 border-t-transparent rounded-full animate-spin"></div></div>
      ) : slots.length === 0 ? (
