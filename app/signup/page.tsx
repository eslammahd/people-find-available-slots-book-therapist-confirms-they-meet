      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-8">
        ...
        <form onSubmit={handleSignup} className="space-y-4">
          <div>
            ...
            <input value={fullName} onChange={e => setFullName(e.target.value)} required className="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-brand-500 outline-none" placeholder="Your full name" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="password" value={password} onChange={e => setPassword(e.target.value)} required minLength={6} className="w-full border border-slate-300 rounded-xl px-4 py-2.5 focus:ring-2 focus:ring-brand-500 outline-none" placeholder="At least 6 characters" />
          </div>
        ...
        <p className="text-center text-sm text-slate-500 mt-4">Already have an account? <a href="/login" className="text-brand-600 font-medium hover:underline">Sign in</a></p>
      </div>
    </div>
  );
}
