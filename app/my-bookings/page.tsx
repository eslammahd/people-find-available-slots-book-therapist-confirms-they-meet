    ...
    <div className="max-w-3xl mx-auto px-4 py-10">
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          {userEmail && <p className="text-sm text-slate-500 mt-0.5">{userEmail}</p>}
        </div>
      ...
      {loading ? (
        <div className="text-center py-20"><div className="inline-block w-8 h-8 border-4 border-sky-600 border-t-transparent rounded-full animate-spin"></div></div>
      ) : bookings.length === 0 ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-slate-200">
          <div className="text-5xl mb-4">&#x1F4C5;</div>
          <p className="text-slate-500 mb-4">You have no bookings yet.</p>
          <a href="/" className="inline-block bg-sky-600 text-white px-6 py-2.5 rounded-xl font-medium hover:bg-sky-700 transition-colors">Book a Session</a>
        </div>
      ) : (
          ...
          {bookings.map(b => {
            ...
            return (
              <div key={b.id} className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
                ...
                {b.status === 'confirmed' && <div className="mt-3 bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-700">Your session is confirmed! Dr. Saad will see you then.</div>}
                {b.status === 'rejected' && <div className="mt-3 bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">This booking was not confirmed. Please book a new slot.</div>}
              </div>
            );
          })}
        </div>
